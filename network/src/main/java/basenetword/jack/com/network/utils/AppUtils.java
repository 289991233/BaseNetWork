...
public final class AppUtils {
    ...
    public static String getVerName(Context context) {
        ...
        try {
            String packageName = context.getPackageName();
            verName = context.getPackageManager().getPackageInfo(packageName, 0).versionName;//+"_"+context.getPackageManager().getPackageInfo(packageName, 0).versionCode;
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }
        return verName;
...
//        // outputX outputY 是裁剪图片宽高
     ...
     * 获取相册的uri图片地址
     */
    public static String getUriImg(Intent data, Context context) {
        Uri selectedImage = data.getData();
        String picturePath = null;
        if (selectedImage != null
                && selectedImage.getScheme() != null
                && selectedImage.getScheme().toString().toLowerCase()
                .startsWith("file")) {
